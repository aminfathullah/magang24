...
def transform_for_display(df):
    # Pilih kolom yang relevan
    df = df[['Tanggal', 'Kabupaten/Kota', 'NAMA BAHAN POKOK', 'HARGA SEKARANG']]
    
    # Filter untuk beras premium dan medium
    df_filtered = df[df['NAMA BAHAN POKOK'].isin([" Beras Premium", " Beras Medium"])]
    
    # Pivot table
    df_pivot = df_filtered.pivot_table(index=['Tanggal', 'Kabupaten/Kota'], columns='NAMA BAHAN POKOK', values='HARGA SEKARANG', aggfunc='first')
...
def main():
    ...
    if all_data:
        ...
        # Transformasi data untuk display
        df_transformed = transform_for_display(df_cleaned)

        # Simpan data ke file Parquet
        pq.write_table(pa.Table.from_pandas(df_transformed), 'beras_looker.parquet')
        ...
        # Buka spreadsheet dan worksheet yang sesuai 
        try:
            gs = gc.open("looker beras")
        ...
        try:
            worksheet.set_dataframe(df_transformed, (1, 1), extend=True, copy_index=False)
