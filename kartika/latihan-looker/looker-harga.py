...
def fetch_data_for_date(date):
    ...
    for kabkota_code, kabkota_name in kabupaten_kota_list.items():
        ...
        if data:
            ...
            # Filter data for "Telur Ayam Ras" and "Telur Ayam Kampung"
            df_filtered = df[df['NAMA BAHAN POKOK'].isin([" Telur Ayam Ras", " Telur Ayam Kampung"])]
...
def main():
    ...
    if all_data:
        ...
        # Buat kolom baru untuk "Telur Ayam Ras" dan " Telur Ayam Kampung"
        final_df['Telur Ayam Ras'] = final_df.apply(lambda x: x['HARGA SEKARANG'] if x['NAMA BAHAN POKOK'] == " Telur Ayam Ras" else None, axis=1)
        final_df['Telur Ayam Kampung'] = final_df.apply(lambda x: x['HARGA SEKARANG'] if x['NAMA BAHAN POKOK'] == " Telur Ayam Kampung" else None, axis=1)
        ...
        # Rename columns
        pivot_df.columns = ['Tanggal', 'Kabupaten/Kota', 'Telur Ayam Kampung', 'Telur Ayam Ras']

        # Fill NaNs with 0s
        pivot_df['Telur Ayam Ras'].fillna(0, inplace=True)
        pivot_df['Telur Ayam Kampung'].fillna(0, inplace=True)

        print("Gabungan Data untuk Semua Tanggal yang berisi Telur Ayam Ras dan Telur Ayam Kampung:")
        print(pivot_df)

        print("Current Working Directory:", os.getcwd())

        file_path = os.path.abspath('data_manipulasi.parquet')
        print("Path ke file Parquet:", file_path)
        
        try:
            gc = pygsheets.authorize(service_account_file='magang-423902-f0e642da9b61.json')
            gs = gc.open('looker-harga')
            worksheet = gs.worksheet('index', 0)
            worksheet.clear('A1')
            worksheet.set_dataframe(pivot_df, (1, 1), extend=True, copy_index=False)
            output_file = 'telur_looker.parquet'
